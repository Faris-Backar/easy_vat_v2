...
class ItemAppbar extends ConsumerStatefulWidget implements PreferredSizeWidget {
  final TextEditingController searchController;
  final ItemAppBarConfig config;
  const ItemAppbar(
      {super.key, required this.searchController, required this.config});

  @override
  ConsumerState<ItemAppbar> createState() => _ItemAppbarState();

  @override
  Size get preferredSize => Size.fromHeight(60.h + kToolbarHeight);
}

class ItemAppBarConfig {
  final String title;
...
class _ItemAppbarState extends ConsumerState<ItemAppbar> {
    ...
    return PopScope(
      ...
      child: AppBar(
        ...
        title: Text(widget.config.title.isNotEmpty
            ? widget.config.title
            : context.translate(AppStrings.items)),
        actions: [
