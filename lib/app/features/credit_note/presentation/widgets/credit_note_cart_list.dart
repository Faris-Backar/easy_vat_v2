...
import 'package:easy_vat_v2/app/features/credit_note/presentation/widgets/credit_note_ledger_card_widget.dart';
...
class CreditNoteCartList extends StatelessWidget {
  final List<CreditNoteCartEntity> ledgerList;
  const CreditNoteCartList({super.key, required this.ledgerList});
  ...
  Widget _buildSlidableLedger(BuildContext context, int index) {
    return Slidable(
      ...
      child: InkWell(
        onTap: () => showDialog(
            context: context,
            builder: (context) => CreditNoteAddDialog(
                  ledger: ledgerList[index].ledger,
                  ledgerEntry: ledgerList[index],
                )),
        child:
            CreditNoteLedgerCardWidget(ledger: ledgerList[index], index: index),
      ),
    );
  }
                      ...
                      return PrimaryButton(
                        ...
                        onPressed: () {
                          ref
                              ...
                              .removeLedgerFromCreditNoteCart(index: index);
