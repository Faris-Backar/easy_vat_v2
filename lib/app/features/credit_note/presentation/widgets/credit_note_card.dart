...
class CreditNoteCard extends StatelessWidget {
  final CreditNoteEntryEntity creditNote;
  final CreditNoteEntryDetailsEntity? creditNoteDetails;
  ...
  Widget build(BuildContext context) {
    final paymentMethod = creditNote.paymentMode?.toLowerCase() ?? "cash";
                ...
                child: Column(
                  children: [
                    Row(
                      children: [
                        ...
                        Expanded(
                          ...
                          children: [
                            Text(creditNote.customerName?.toString() ?? "Cash",
                            ...
                            SizedBox(height: 2.h),
                            Text(
                              "${context.translate(AppStrings.creditNoteNo)}: ${creditNote.creditNoteNo ?? "-"}",
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                                "₹ ${creditNote.totalAmount?.toStringAsFixed(2) ?? "0.00"}",
                    ...
                    if (isSelected) ...[
                      ...
                      Divider(
                          color:
                              context.defaultTextColor.withValues(alpha: 0.1),
                          thickness: 1),
                      SizedBox(height: 8.h),
      ...
      children: [
        if (isTaxEnabled) ...[
          _buildDetailRow(context,
              ...
              value:
                  "₹ ${creditNoteDetails?.drAmount?.toStringAsFixed(2) ?? "0.00"}"),
          SizedBox(height: 4.h),
        ],
        _buildDetailRow(context,
            label: context.translate(AppStrings.totalTax),
            value: "₹ ${creditNoteDetails?.tax?.toStringAsFixed(2) ?? "0.00"}"),
        SizedBox(height: 4.h),
        _buildDetailRow(context,
            label: context.translate(AppStrings.netTotal),
            value: "₹ ${creditNote.totalAmount?.toStringAsFixed(2) ?? "0.00"}")
      ],
    );
  }
