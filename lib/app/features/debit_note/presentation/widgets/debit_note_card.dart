...
class DebitNoteCard extends StatelessWidget {
  final DebitNoteEntryEntity debitNote;
  final DebitNoteEntryDetailsEntity? debitNoteEntry;
  ...
  Widget build(BuildContext context) {
    final paymentMethod = debitNote.paymentMode?.toLowerCase() ?? "cash";
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              debitNote.supplierName?.toString() ?? "Cash",
                            ...
                            Text(
                              "${context.translate(AppStrings.debitNoteNo)}: ${debitNote.debitNoteNo ?? "-"}   ${context.translate(AppStrings.supRefNo)}: ${debitNote.supplierReferenceNo ?? ""}",
                      ...
                      Column(
                        ...
                        children: [
                          Text(
                            "₹ ${debitNote.totalAmount?.toStringAsFixed(2) ?? "0.00"}",
      ...
      children: [
        if (isTaxEnabled) ...[
          _buildDetailRow(context,
              ...
              value:
                  "₹ ${debitNoteEntry?.crAmount?.toStringAsFixed(2) ?? "0.00"}"),
          SizedBox(
            height: 4.h,
          ),
        ],
        _buildDetailRow(
          ...
          value: "₹ ${debitNoteEntry?.tax?.toStringAsFixed(2) ?? "0.00"}",
        ),
        SizedBox(
          height: 4.h,
        ),
        _buildDetailRow(
          ...
          value: "₹ ${debitNote.totalAmount?.toStringAsFixed(2) ?? "0.00"}",
          isTotal: true,
        ),
      ],
    );
