...
class IncomeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final paymentMethod = income.paymentMode?.toLowerCase() ?? "cash";
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          ...
                          children: [
                            Text(income.customerName?.toString() ?? "Cash",
                            ...
                            SizedBox(height: 2.h),
                            Text(
                              "${context.translate(AppStrings.incNo)}: ${income.incomeNo ?? "-"}",
                        ...
                        Column(
                          ...
                          children: [
                            Text(
                                "₹ ${income.netTotal?.toStringAsFixed(2) ?? "0.00"}",
      ...
      children: [
        if (isTaxEnabled) ...[
          _buildDetailRow(context,
              ...
              value: "₹ ${income.grossTotal?.toStringAsFixed(2) ?? "0.00"}"),
          SizedBox(height: 4.h),
        ],
        _buildDetailRow(context,
            label: context.translate(AppStrings.totalTax),
            value: "₹ ${income.tax?.toStringAsFixed(2) ?? "0.00"}"),
        SizedBox(height: 4.h),
        _buildDetailRow(context,
            label: context.translate(AppStrings.discount),
            value: "₹ ${income.discount?.toStringAsFixed(2) ?? "0.0"}",
            isTotal: true),
        _buildDetailRow(context,
            label: context.translate(AppStrings.netTotal),
            value: "₹ ${income.netTotal?.toStringAsFixed(2) ?? "0.00"}")
      ],
    );
  }
