...
class IncomeAddDialog extends ConsumerStatefulWidget {
  final LedgerAccountEntity ledger;
  final IncomeCartEntity? ledgerEntry;
  const IncomeAddDialog({super.key, required this.ledger, this.ledgerEntry});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _IncomeAddDialogState();
}

class _IncomeAddDialogState extends ConsumerState<IncomeAddDialog> {
  IncomeCartEntity? cart;
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        PrimaryButton(
          ...
          onPressed: _handleIncomeCartAction,
          // income cart acion
        )
      ],
    );
  ...
  void _handleIncomeCartAction() {
    final incomeCartNotifier = ref.read(incomeCartProvider.notifier);
    final grossTotal = double.tryParse(_grossTotalController.text) ?? 0.0;
    final discount = double.tryParse(_discountController.text) ?? 0.0;
    final taxPercentage = double.tryParse(_taxPercentController.text) ?? 0.0;
    final taxAmount = incomeCartNotifier.calculateTotalTax(
        grossTotal: grossTotal, taxPercentage: taxPercentage);
    final netTotal = grossTotal + taxAmount;

    final incomeCartEntity = IncomeCartEntity(
        ledgerId: widget.ledgerEntry != null
            ? widget.ledgerEntry!.ledgerId
            : (ref.read(incomeCartProvider).ledgerList?.length ?? 0) + 1,
    ...
    if (widget.ledgerEntry == null) {
      incomeCartNotifier.addLedgerIntoIncomeCart(ledger: incomeCartEntity);
    } else {
      incomeCartNotifier.updateIncomeCartLedger(cartLedger: incomeCartEntity);
    }
