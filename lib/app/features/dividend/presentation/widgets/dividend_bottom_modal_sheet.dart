...
class DividendBottomModalSheet extends ConsumerStatefulWidget {
  final LedgerAccountEntity? ledger;
  const DividendBottomModalSheet({super.key, this.ledger});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _DividendBottomModalSheetState();
}

class _DividendBottomModalSheetState
    extends ConsumerState<DividendBottomModalSheet> {
  final TextEditingController searchController = TextEditingController();
  final ValueNotifier<int?> dividendDetailsExpansionNotifier =
      ValueNotifier(null);

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(capitalLedgerNotifierProvider);
    return Container(
      height: 1.sh,
          ...
          Expanded(
            child: state.map(
                ...
                loaded: (ledgerList) => ledgerList.ledgers.isEmpty
                    ...
                    : _buildCapitalLedgerList(ledgerList.ledgers),
...
Future<void> showCapitalLedgerBottomSheet(
  ...
  await showModalBottomSheet(
      ...
      builder: (context) {
        return DividendBottomModalSheet();
      });
}
