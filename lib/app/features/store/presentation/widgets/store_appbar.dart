...
class StoreAppbar extends ConsumerStatefulWidget
    implements PreferredSizeWidget {
  final TextEditingController searchController;
  final StoreAppBarConfig config;
  const StoreAppbar(
      {super.key, required this.searchController, required this.config});

  @override
  ConsumerState<StoreAppbar> createState() => _StoreAppbarState();

  @override
  Size get preferredSize => Size.fromHeight(60.h + kToolbarHeight);
}

class StoreAppBarConfig {
  final String title;
...
class _StoreAppbarState extends ConsumerState<StoreAppbar> {
    ...
    return PopScope(
      ...
      child: AppBar(
        ...
        title: Text(widget.config.title.isNotEmpty
            ? widget.config.title
            : context.translate(AppStrings.store)),
        actions: [
